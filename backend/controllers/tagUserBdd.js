...
async function getTagUser(req, res) {//apiToken
    ...
    async function getTagNotUser(req, res) {
      ...
      try {
        ...
        const unlinkedTagNamesOnly = unlinkedTagNames.map((kpiName) => kpiName.name);
    ...
    async function addTagToUser(req, res) {
      ...
      async function removeTagFromUser(req, res) {
        ...
        try {
          ...
          if (!user || !tagName) {
            return res.status(404).json({ message: "Utilisateur ou tagName non trouvé." });
          }
