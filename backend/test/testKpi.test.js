...
describe("KPI", () => {
  ...
  describe("getKPI", () => {
    it("avec param", async () => {
      ...
      chai.request(server).get("/kpi").send(req.body).set("Cookie", [`token=${token}; ${cookieOptions}`]).end((err, res) => {
        expect(res).to.have.status(200);
        done(); // Appelez done() ici pour indiquer que le test est terminé
      });
  ...
  describe("KpiChoice", () => {
    it("avec param", async () => {
      ...
      chai.request(server).post("/kpi/post/:nameCard/:resperiod").send(req.params, req.body).set("Cookie", [`token=${token}; ${cookieOptions}`]) .end((err, res) => {
        expect(res).to.have.status(200);
        done(); // Appelez done() ici pour indiquer que le test est terminé
      });
    });
    it("sans body", async () => {
        ...
        chai.request(server).post("/kpi/post/:nameCard/:resperiod").send(req.params).set("Cookie", [`token=${token}; ${cookieOptions}`]) .end((err, res) => {
          expect(res).to.have.status(200);
          done(); // Appelez done() ici pour indiquer que le test est terminé
        });
      });
      it("sans param", async () => {
        ...
        const { token, cookieOptions } = generateJWTAndCookie(user);
         ...
         chai.request(server).post("/kpi/post/:nameCard/:resperiod").set("Cookie", [`token=${token}; ${cookieOptions}`]) .end((err, res) => {
          expect(res).to.have.status(200);
          done(); // Appelez done() ici pour indiquer que le test est terminé
        });
      });
  ...
  describe("getKpiUser BDD", () => {
    it("avec info", async () => {
      ...
      const res = await chai.request(server).get("/kpi/bdd/:id").send(req.params).set("Cookie", [`token=${token}; ${cookieOptions}`]);
  ...
  describe("getKpiNotUser BDD", () => {
    it("avec info", async () => {
      ...
      const res = await chai.request(server).get("/kpi/bddnot/:id").send(req.params).set("Cookie", [`token=${token}; ${cookieOptions}`]);
